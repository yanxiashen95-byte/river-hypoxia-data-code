...
# =========================
plot_df <- df %>%
  select(solar_p75, mean_duration) %>%
  mutate(
    solar_p75 = as.numeric(solar_p75),
    mean_duration = as.numeric(mean_duration)
  ) %>%
  filter(!is.na(solar_p75), !is.na(mean_duration))

# =========================
# 3. 拟合 GAM
# =========================
gam_mod <- gam(
  mean_duration ~ s(solar_p75, k = 5),
  data = plot_df,
  method = "REML"
)
...
# 6. 画图
# =========================
p1 <- ggplot(plot_df, aes(x = solar_p75, y = mean_duration)) +
  geom_point(
    color = point_color,
    size = point_size,
    alpha = point_alpha
...
ggsave("GAM_solar_mean_duration.png", p1, width = 7.5, height = 5.8, dpi = 300)
