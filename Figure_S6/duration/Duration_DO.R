...
# =========================
plot_df <- df %>%
  select(DO_P25, mean_duration) %>%
  mutate(
    DO_P25 = as.numeric(DO_P25),
    mean_duration = as.numeric(mean_duration)
  ) %>%
  filter(!is.na(DO_P25), !is.na(mean_duration))

# =========================
# 3. 拟合 GAM
# =========================
gam_mod <- gam(
  mean_duration ~ s(DO_P25, k = 5),
  data = plot_df,
  method = "REML"
)
...
# 6. 画图
# =========================
p1 <- ggplot(plot_df, aes(x = DO_P25, y = mean_duration)) +
  geom_point(
    color = point_color,
    size = point_size,
    alpha = point_alpha
...
ggsave("GAM_DO_mean_duration.png", p1, width = 7.5, height = 5.8, dpi = 300)