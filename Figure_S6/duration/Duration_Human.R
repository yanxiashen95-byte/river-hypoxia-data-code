...
# =========================
plot_df <- df %>%
  select(hdi_ix_cav, mean_duration) %>%
  mutate(
    hdi_ix_cav = as.numeric(hdi_ix_cav),
    mean_duration = as.numeric(mean_duration)
  ) %>%
  filter(!is.na(hdi_ix_cav), !is.na(mean_duration))

# =========================
# 3. 拟合 GAM
# =========================
gam_mod <- gam(
  mean_duration ~ s(hdi_ix_cav, k = 5),
  data = plot_df,
  method = "REML"
)
...
# 6. 画图
# =========================
p1 <- ggplot(plot_df, aes(x = hdi_ix_cav, y = mean_duration)) +
  geom_point(
    color = point_color,
    size = point_size,
    alpha = point_alpha
...
ggsave("GAM_Human_mean_duration.png", p1, width = 7.5, height = 5.8, dpi = 300)
