...
# =========================
plot_df <- df %>%
  select(WT_p75, hypoxia_frequency) %>%
  mutate(
    WT_p75 = as.numeric(WT_p75),
    mean_duration = as.numeric(hypoxia_frequency)
  ) %>%
  filter(!is.na(WT_p75), !is.na(hypoxia_frequency))

# =========================
# 3. 拟合 GAM
# =========================
gam_mod <- gam(
  hypoxia_frequency ~ s(WT_p75, k = 5),
  data = plot_df,
  method = "REML"
)
...
# 6. 画图
# =========================
p1 <- ggplot(plot_df, aes(x = WT_p75, y = hypoxia_frequency)) +
  geom_point(
    color = point_color,
    size = point_size,
    alpha = point_alpha
...
ggsave("GAM_WT_hypoxia_frequency.png", p1, width = 7.5, height = 5.8, dpi = 300)